...
window.onload = function() {
  // openTab(new Event('click'), 'colorTab'); // Replace 'colorTab' with the ID of your default tab
...
// Add this at the beginning of your index.js file
function initialize() {
  openTab(new Event('click'), 'colorTab');
  loadButtons();
  colorPickerFunc();
  loadIMGButtons();
...
function setupFileInputListener() {
  ...
  fileInput.addEventListener('change', function(e) {
    ...
    reader.onload = function(e) {
      // ... the rest of your file reader code ...
    };
...
// Use addEventListener instead of window.onload to avoid overwriting.
...
function makeAndTake() {
  html2canvas(document.getElementById("borderImageID"), {
      // scale: window.devicePixelRatio, // Captures the canvas at the device's resolution
      backgroundColor: null
  }).then(function(canvas) {
      var dataURL = canvas.toDataURL("image/png");
      downloadImage(dataURL, 'custom-image.png');
  });
}
...
function openTab(evt, tabCorI) {
  ...
  for (i = 0; i < tablinks.length; i++) {
    ...
    // Remove underline from all tab headers
    tablinks[i].querySelector('h2').style.textDecoration = 'none';
  }

  // Show the current tab, and add an "active" class to the button that opened the tab
  document.getElementById(tabCorI).style.display = "block";
  evt.currentTarget.classList.add("active");
  // Apply underline to the active tab header
  evt.currentTarget.querySelector('h2').style.textDecoration = 'underline';
}
